...
class sensorCrypt(object):
        def __init__(self, vCryptKey):
            self.__vSalt = 'Qr1#Y'
            self.__lenKey = len(vCryptKey+self.__vSalt)
            self.__crypt_position = self.__GetCryptPosition()
            self.__key = hashlib.sha512(str(vCryptKey+self.__vSalt+str(self.__crypt_position))).hexdigest()
        ...
        def Decrypt(self, vString):
            ...
            vBase64 = vString[0:self.__crypt_position] + vString[self.__crypt_position:len(vString)]
            # print vBase64
